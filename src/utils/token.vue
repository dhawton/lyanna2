  ...
  methods: {
    checkToken() {
      ...
      if (
        ...
        Math.floor(new Date().getTime() / 1000) > window.localStorage.getItem('te')
      ) {
        this.$router.push({ path: '/login' });
        return;
      }

      if (Math.floor(new Date().getTime() / 1000) >= window.localStorage.getItem('te') - 600) {
        this.refreshToken();
      }
    },
    refreshToken() {
